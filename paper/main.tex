...
\maketitle
\begin{abstract}
A large literature documents hundreds of stock characteristics with predictive content, but it is unclear whether this high-dimensional characteristic zoo represents many distinct investment opportunities or a much lower-dimensional nonlinear structure. We estimate that structure directly from an investor's objective. A neural network maps firm characteristics into a small collection of characteristic-managed portfolios and combines them using aggregate market conditions and a learned summary of the current cross-section. The entire system is trained end-to-end to recover the conditional tangency portfolio, without first forecasting each stock's expected return and without pre-specifying factor definitions. We introduce a rotation-invariant economic allocation spectrum that orders the learned factor space by its use in the portfolio and yields an endogenous effective factor dimension. In U.S. equities from 1963 to 2024, the model compresses 123 firm characteristics, a size rank, and their nonlinear interactions into a small number of economically active directions. The leading factor combines momentum, profitability and quality, earnings information, growth, and avoidance of extreme recent winners, while its allocation varies systematically with market volatility and cross-sectional dispersion. The learned factor remains economically important after spanning by standard factor models. These findings suggest that the cross-section can be high-dimensional in observables while remaining low-dimensional in the nonlinear combinations that matter for portfolio choice.
\end{abstract}
...
A generic neural network could be trained to minimize stock-level forecast errors and then passed to a separate optimizer. We instead train the entire architecture on the final portfolio. For any portfolio payoff $R^p$, define
\begin{equation}
...
Table~\ref{tab:baseline} reports the current fixed-split evidence. The representative $K_{\max}=32$ model achieves an annualized holdout Sharpe ratio of \BaselineTestSharpe, while an equal-weighted ensemble across ten random initializations reaches 1.49. A linear managed-portfolio benchmark reaches 1.53 over the same 2015--2024 test period. Thus the present evidence does not support a claim that nonlinearity raises average holdout Sharpe on the long sample; its stronger contribution is to organize a rich characteristic space into a low-dimensional investment policy. The annual expanding-window exercise is implemented and is reported separately once complete.
...
The first main result is a sharp compression of the factor space. Although the baseline architecture permits multiple latent factors, the economic allocation spectrum is strongly concentrated. The entropy effective dimension is \KEff, and the first canonical direction accounts for \FirstShare\ of total allocation intensity. Figure~\ref{fig:spectrum} plots the spectrum.
\begin{figure}[t]
\centering
\IfFileExists{figures/economic_spectrum.pdf}{\includegraphics[width=0.72\textwidth]{figures/economic_spectrum.pdf}}{\fbox{\parbox[c][2.2in][c]{0.68\textwidth}{\centering Economic allocation spectrum placeholder}}}
\caption{Economic allocation spectrum. The figure reports the normalized eigenvalues of the canonical allocation operator in \eqref{eq:economic_operator}.}
\label{fig:spectrum}
\end{figure}

The distinction between $K$ and $K_{\mathrm{eff}}$ is economically useful. A network with many parameters and a large latent bottleneck can still learn that only a small number of characteristic-managed portfolios deserve material allocation. Rich functional approximation and parsimonious economic structure are therefore complements rather than substitutes.

The $K$-sweep makes this point directly. If additional factors uncover genuinely independent opportunities, holdout performance and $K_{\mathrm{eff}}$ should rise together. If they merely offer redundant coordinates, nominal capacity rises while the economic spectrum remains concentrated. Table~\ref{tab:kgrid} reports the distribution across random seeds.

\begin{table}[t]
\centering
\caption{Maximum Factor Capacity and Economic Dimension}\label{tab:kgrid}
\input{tables/k_grid.tex}
\end{table}
...
\input{tables/factor_profile.tex}
\end{table}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/interactions.pdf}{\includegraphics[width=0.82\textwidth]{figures/interactions.pdf}}{\fbox{\parbox[c][2.2in][c]{0.78\textwidth}{\centering Characteristic-interaction figure placeholder}}}
...
\input{tables/marginal_sharpe.tex}
\end{table}
...
We estimate five economically targeted ablations. The first removes aggregate market state $M_t$ but retains the learned cross-sectional state. The second removes cross-sectional context while retaining aggregate state. The third uses a static allocator. The fourth makes the characteristic map linear while keeping the same conditional allocator, isolating the value of nonlinear feature learning. Finally, we estimate a direct conditional neural policy in which the stock score may depend jointly on firm characteristics, cross-sectional context, and aggregate state. The last comparison measures the performance cost, if any, of insisting on an interpretable separable factor bottleneck.
\begin{table}[t]
\centering
...
The walk-forward protocol and code are included in the replication package. The final concatenated return series is intentionally left unreported in this snapshot because the expanding-window run is still executing. Figure~\ref{fig:walkforward} is populated only after that run completes. The walk-forward exercise is economically important because it exposes the model to changes in the cross-sectional composition of the market, characteristic distributions, and aggregate states without allowing future information to influence training.

\begin{figure}[t]
\centering
\IfFileExists{figures/walk_forward.pdf}{\includegraphics[width=0.82\textwidth]{figures/walk_forward.pdf}}{\fbox{\parbox[c][2.2in][c]{0.78\textwidth}{\centering Walk-forward performance placeholder}}}
\caption{Expanding-window walk-forward performance. Each year's portfolio is estimated without using that year's returns.}
\label{fig:walkforward}
\end{figure}

\subsection{Stability of factor interactions and economic dimension}
We finally ask whether the dominant nonlinear factor is a stable economic object or a feature of one subperiod. We measure $K_{\mathrm{eff}}$ separately in early training, late training, validation, and test samples. We also hold a common set of leading characteristics fixed and re-estimate the pairwise interaction diagnostics within each subperiod.
If the learned factor is stable, the rankings of interaction strengths should be positively associated across subperiods and the effective dimension should remain small even as the nominal network capacity is unchanged. Table~\ref{tab:stability} reports entropy rank, the share of the first canonical direction, Spearman rank correlations of interaction scores, and overlap among the strongest interactions.
